  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex sm:justify-center items-start justify-end">
      ...
      <div className="relative -bottom-30 -right-10 sm:bottom-0 sm:right-0 flex flex-col justify-end items-end sm:justify-center sm:items-center pointer-events-none">
        {/* === Basketball === */}
        {/* 1️⃣ Faza – piłka delikatnie zostaje */}
        <Parallax translateY={[0, 100, "easeOutQuad"]} startScroll={0} endScroll={400}>
          {/* 2️⃣ Faza – piłka leci w górę */}
          <Parallax translateY={[0, -500, "easeInQuint"]} startScroll={600} endScroll={700}>
            <div className="relative bottom-0 sm:-bottom-5 w-70 sm:w-110">
              ...
              {/* === Masking watermark === */}
              <div
                className={`absolute bottom-1 right-0 h-[15%] w-[30%] transition-colors duration-700 ${isScrolled ? "bg-second" : "bg-first"
                  }`}
              />
            </div>
          </Parallax>
        ...
        {/* === Hand === */}
        <Parallax translateY={[0, 800, "easeInQuint"]} startScroll={0} endScroll={100}>
